...
last_frame = 1828
cap.set(cv2.cv.CV_CAP_PROP_POS_FRAMES, last_frame)
...
while(1):
    count += 1
    img = frame#cv2.imread('FIRST_FRAME.PNG',1)
...
mask = cv2.inRange(hsv_roi, np.array((90,0,0)), np.array((125,255,255)))
...
count=0
while(last_frame < cap.get(cv2.cv.CV_CAP_PROP_FRAME_COUNT)):
    ret ,frame = cap.read()
    count+=1
    last_frame+=1
    if ret == True:
