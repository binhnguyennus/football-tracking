...
r,h,c,w = 286,17,2263,7  # simply hardcoded the values player2 

#row, hight of the window, col, width of the window
#ddddd =  cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
ddddd =  cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
for i in range(286,303):
    for j in range(2263,2270):
        print i,"  ",j,"  ",ddddd[i,j,:]
        
#meann= np.mean(ddddd, axis=1)        
#
#stddeviation = np.std(ddddd, axis=1)
...
mask = cv2.inRange(hsv_roi, np.array((48., 0.,0.)), np.array((121.,255.,255.)))
